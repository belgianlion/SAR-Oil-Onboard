...
def main():
    ...
    parser.add_argument("-p", "--polarization", type=str, help="Specify the input UAVSAR polarization in UPPERCASE (i.e HHHV, HHHH, VVVV, AMP1, AMP2, COR, UNW, HGT, INT)")
    args = parser.parse_args()

    if str(args.input).endswith('.txt'):
        pass
    elif str(args.input).endswith('.ann'):
        pass
    else:
        print("INPUT UAVSAR ANN FILE MUST END '.txt' OR '.ann'")
        os._exit(1)
    if args.input == None:
        print("SPECIFY INPUT TXT FILE")
        os._exit(1)
    elif args.uavsar == None:
        print("SPECIFY INPUT UAVSAR FILE")
        os._exit(1)
    elif args.polarization == None:
        print("SPECIFY UAVSAR IMAGE POLARIZATION (i.e. HHHV, HHHH, VVVV, AMP1, AMP2, COR, UNW, HGT, INT)")
        os._exit(1)
